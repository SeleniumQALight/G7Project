...
public class ExcelDriver {
	 ...
	 */
//сигнатура метода: вхідні параметри: шлях, назва вкладки, з якого рядочка вичитка
    ...
    public static Map<String, String> getData(String dataFileName, String sheetName) throws IOException {
        return getMultipleData(dataFileName, sheetName, 1);// починається з 0
    }
	 ...
	 * data values in the test.
	 */
    //зчитує key-value pairs коли в рядочок читаєм
	public static Map getDataRow(String dataFileName, String sheetName) throws IOException {
		...
		for (int k = 0; k < (dataSize); k++) {
			HSSFCell keyCell = sheet.getRow(2).getCell(k);// рядок колонка
			HSSFCell valueCell = sheet.getRow(3).getCell(k);
			valueCell.setCellType(HSSFCell.CELL_TYPE_STRING);//перероблюємо в тип стрінг
			testData.put(keyCell.getStringCellValue(), valueCell.getStringCellValue());//записуємо в хешмапку ключ значення
		}
