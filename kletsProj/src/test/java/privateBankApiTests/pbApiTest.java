...
import static privateBankApi.dto.responseDto.ExchangeRateDto.createExRateWithUAHAsDefault;
...
//        }
        ...
        List<ExchangeRateDto> listOfAllExchangeRates = new ArrayList<>();

        List<String> listOfAllCurrencies = List.of("AUD", "AZN", "BYN", "CAD", "CHF", "CNY", "CZK", "DKK", "EUR",
                "GBP", "GEL", "HUF", "ILS", "JPY", "KZT", "MDL", "NOK", "PLN", "SEK", "SGD", "TMT", "TRY", "UAH",
                "USD", "UZS", "XDR", "ZAR");

        for (String currency : listOfAllCurrencies) {
            listOfAllExchangeRates.add(createExRateWithUAHAsDefault(currency));
        }
        ...
        CurrencyDto expectedResult =
                CurrencyDto.builder()
                        ...
                        .bank("PB")
                        .baseCurrency(980)
                        .baseCurrencyLit(BASE_CURRENCY)
                        .exchangeRate(listOfAllExchangeRates)
                        .build();
        Assert.assertEquals(expectedResult.getExchangeRate().size(), currencyResponseAsDto.getExchangeRate().size());

        SoftAssertions softAssertions = new SoftAssertions();

        softAssertions.assertThat(currencyResponseAsDto)
